...
describe('Campaigns', () => {
  ...
  it('allows people to contribute money and marks them as approvers', async () => {
    ...
    const isContriburer = await campaign.methods
      .approvers(accounts[1])
      .call();
    assert.ok(isContriburer);
  });
  ...
  it('requires a minimum contribution', async () => {
    try {
      await campaign.methods
        .contribure()
        .send({ from: accounts[1], gas: 1000000, value: 50 });
      assert(false);
    } catch (error) {
      assert(error);
    }
  ...
  it('allows a manager to make a payment request', async () => {
    await campaign.methods
      .createRequest('Test reqeust', 1000, accounts[0])
      .send({ from: accounts[0], gas: 1000000 });
  ...
  it('processes requests', async () => {
    ...
    const firstBalance = web3.utils.fromWei(
    ...
    assert(finalBalance - 4 > firstBalance);
  });
});
